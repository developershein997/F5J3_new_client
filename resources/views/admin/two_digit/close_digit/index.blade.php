...
@section('style')
    <style>
        ...
        .digit-item {
            ...
            justify-content: center;
            border-radius: 12px;
            box-shadow: 0 2px 8px rgba(255, 255, 255, 0.938);
            margin-bottom: 10px;
            font-size: 1.5rem;
            font-weight: bold;
        ...
        .digit-box-modern {
            background: #23272f;
            border-radius: 10px;
            box-shadow: 0 2px 8px rgba(255, 251, 251, 0.08);
        ...
        .horizontal-bar-modern {
            display: flex;
            gap: 10px;
            /* Space between individual digit boxes */
            flex-wrap: wrap;
            /* Allow wrapping if many items in a chunk */
        }
        ...
        .digit-box-modern {
            ...
            justify-content: center;
            min-width: 150px;
            /* Make them wider for battle names */
            height: 120px;
            /* Adjust height */
            position: relative;
            overflow: hidden;
        ...
        .toggle-dot {
            ...
            /* Default to left for inactive */
        }

        .digit-box-modern.active .toggle-dot {
            transform: translateX(12px);
            /* Move to right for active */
        }
    </style>
@endsection
...
@section('content')
                            ...
                            <div class="row mt-4"> {{-- Added margin-top for spacing --}}
                                <div class="col-lg-4 col-md-4 col-sm-6">
                                    ...
                                    <div class="horizontal-bar-group">
                                        @foreach ($battles->chunk(2) as $chunk)
                                            {{-- Chunk by 2 since you have two main periods --}}
                                            <div class="horizontal-bar-modern">
                                                @foreach ($chunk as $battle)
                                                    <div class="digit-box-modern {{ $battle->status ? 'active' : '' }}"
                                                        data-id="{{ $battle->id }}" data-status="{{ $battle->status }}"
                                ...
                                <div class="col-lg-4 col-md-4 col-sm-6">
                                    ...
                                    <div class="horizontal-bar-group">
                                        @if ($twoDLimit)
                                            {{-- Check if $twoDLimit is not null --}}
                                ...
                                <div class="col-lg-4 col-md-4 col-sm-6">
                                    ...
                                    <div class="d-flex flex-wrap justify-content-center align-items-end gap-3 mb-3 ">
                                        <div class="digit-box-modern text-center py-4 px-2 mx-2 mb-2"
                                            style="background: #1a1a2e; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.08); max-width: 200px; min-width: 160px; flex: 1 1 180px;">
                                            <h6 class="text-black mb-2">Win Number</h6>
                                            <p class="font-weight-bold  mb-0"
                                                style="color: #1d1b12; word-break: break-all;font-size:10px">
                                                @if ($twoDResult)
                                        ...
                                        <div class="digit-box-modern text-center py-4 px-2 mx-2 mb-2"
                                            style="background: #16213e; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.08); max-width: 200px; min-width: 160px; flex: 1 1 180px;">
                                            <h6 class="text-black mb-2">Session</h6>
                                            <p class="font-weight-bold  mb-0"
                                                style="color: #1d1b12; word-break: break-all;font-size:10px">
                                                @if ($twoDResult)
                                        ...
                                        <div class="digit-box-modern text-center py-4 px-2 mx-2 mb-2"
                                            style="background: #0f3460; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.08); max-width: 200px; min-width: 160px; flex: 1 1 180px;">
                                            <h6 class="text-black mb-2">Result Date</h6>
                                            <p class="font-weight-bold  mb-0"
                                                   style="color: #1d1b12; word-break: break-all;font-size:10px">
                                                @if ($twoDResult)
  ...
  <div class="d-flex justify-content-center mt-3">
                        ...
                        <div class="card-body justify-content-center">
                            ...
                            <div class="head-digits-container">
                                ...
                                <div class="d-flex justify-content-center">
                                        ...
                                        <div class="digit-item col-lg-2 col-md-2 col-sm-2 col-2 mx-1 {{ $digit->status ? 'active' : 'inactive' }}"
                                            data-id="{{ $digit->id }}">
                                            ...
                                            <div class="digit-status">
                                                <span
                                                    class="status-text {{ $digit->status ? 'text-success' : 'text-danger' }}">
                                                    {{ $digit->status ? 'ON' : 'OFF' }}
                                                </span>
                                            </div>
                                        </div>
                                    ...
                                    <div class="horizontal-bar-modern">
                                        @foreach ($chunk as $digit)
                                            <div class="digit-box-modern {{ $digit->status ? 'active' : 'inactive' }}"
                                                data-id="{{ $digit->id }}" data-status="{{ $digit->status }}"
                                                onclick="toggleChooseDigitStatus(this)" title="Click to toggle status">
    ...
    <!-- TwoD Result Modal -->
    <div class="modal fade" id="resultDigitModal" tabindex="-1" role="dialog" aria-labelledby="resultDigitModalLabel"
        aria-modal="true">
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control @error('two_d_result') is-invalid @enderror"
                                id="two_d_result" name="two_d_result" placeholder="Enter TwoD Result" required
                                aria-required="true">
                            @error('two_d_result')
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control @error('session') is-invalid @enderror" id="session"
                                name="session" required aria-required="true">
    ...
    <div class="modal fade" id="headCloseDigitModal" tabindex="-1" role="dialog"
        aria-labelledby="headCloseDigitModalLabel" aria-modal="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            ...
                            <input type="number" class="form-control @error('two_d_limit') is-invalid @enderror"
                                id="two_d_limit" name="two_d_limit" placeholder="Enter 2D Limit (Break)" required
                                aria-required="true">
                            @error('two_d_limit')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
            ...
            $('.status-toggle').on('change', function() {
                ...
                $.ajax({
                    ...
                    error: function(xhr) {
                        ...
                        if (!isChecked) {
                            digitItem.addClass('active').removeClass('inactive');
                            statusText.removeClass('text-danger').addClass('text-success').text(
                                'ON');
                        } else {
                            digitItem.removeClass('active').addClass('inactive');
                            statusText.removeClass('text-success').addClass('text-danger').text(
                                'OFF');
                        }
                ...
                .then(data => {
                    if (!data.success) {
                        ...
                        showMessageBox(data.message ||
                            'စိတ်ကြိုက် ပိတ်ဂဏန်းပိတ်သိမ်းမှု့အောင်မြင်ပါသည် | Status updated successfully!',
                            'success');
                    }
