...
<section class="content-header">
    <div class="container-fluid">
        <div class="row mb-2 align-items-center">
            <div class="col-sm-8">
                <h1 class="mb-3">3D Bet Slip Details</h1>
                <div class="card bg-light mb-3 shadow-sm">
                    <div class="card-body d-flex flex-wrap align-items-center">
                        ...
                        <div class="mr-4 mb-2">
                            <span class="text-secondary"><i class="fa fa-calendar"></i> <strong>Draw Session:</strong></span>
                            <span class="ml-1">{{ $slip->draw_session }}</span>
                        </div>
                        <div class="mr-4 mb-2">
                            <span class="text-secondary"><i class="fa fa-info-circle"></i> <strong>Status:</strong></span>
                            <span class="ml-1 badge badge-{{ $slip->status == 'pending' ? 'warning' : ($slip->status == 'completed' ? 'success' : 'secondary') }}">
                                {{ ucfirst($slip->status) }}
                            </span>
                        </div>
                        <div class="mb-2">
            ...
            <div class="col-sm-4 text-right">
                <a href="{{ route('admin.threed.bet-slip-list') }}" class="btn btn-outline-primary mt-2">
    ...
    <div class="container-fluid">
        <div class="card shadow">
            <div class="card-header bg-primary text-white">
                <h5 class="mb-0"><i class="fa fa-list-ol"></i> 3D Bets in this Slip</h5>
            </div>
            <div class="card-body">
                            ...
                            <tbody>
                                @foreach($bets as $i => $bet)
                                    <tr>
                                        ...
                                        <td>
                                            @if($bet->is_permutation)
                                                <span class="badge badge-info">Yes</span>
                                                @if($bet->permutation_amount)
                                                    <br><small class="text-muted">{{ number_format($bet->permutation_amount, 2) }}</small>
                                                @endif
                                            @else
                                        ...
                                        <td>
                                            <span class="badge badge-warning">Break {{ $bet->break_group }}</span>
                                        </td>
                                        <td>
                                            @if($bet->win_lose)
                                                <span class="badge badge-success"><i class="fa fa-trophy"></i> Win</span>
                                                @if($bet->potential_payout)
                                                    <br><small class="text-success">{{ number_format($bet->potential_payout, 2) }}</small>
                                                @endif
                                            @else
