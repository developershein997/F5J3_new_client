            ...
            <div class="col-sm-6">
                <form class="form-inline float-sm-right" method="GET">
                    ...
                    <select name="draw_session" class="form-control mr-2">
                        <option value="">All Draw Sessions</option>
                        ...
                        <tbody>
                            @forelse($slips as $slip)
                                <tr>
                                    ...
                                    <td>
                                        <span class="badge badge-{{ $slip->status == 'pending' ? 'warning' : ($slip->status == 'completed' ? 'success' : 'secondary') }}">
                                            {{ ucfirst($slip->status) }}
                                        </span>
                                    </td>
                                    <td>{{ $slip->draw_session }}</td>
                                    <td>{{ $slip->created_at->format('Y-m-d H:i') }}</td>
                                    <td>
                                        <a href="{{ route('admin.threed.bet-slip-details', $slip->id) }}" class="btn btn-sm btn-info" title="View Details">
                                            <i class="fa fa-eye"></i>
                                        </a>
                                    </td>
                                </tr>
