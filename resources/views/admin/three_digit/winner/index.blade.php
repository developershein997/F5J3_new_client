        ...
        <div class="row mb-2 align-items-center">
            ...
            <div class="col-sm-6">
                <form class="form-inline float-sm-right" method="GET">
                    ...
                    <select name="draw_session" class="form-control mr-2">
                        <option value="all" {{ request('draw_session') == 'all' ? 'selected' : '' }}>All Draw Sessions</option>
...
<section class="content">
    <div class="container-fluid">
        @if(isset($result) && is_array($result))
            <!-- Multiple Draw Sessions View (All Sessions) -->
            <div class="row">
                @foreach($result as $session => $sessionData)
                    <div class="col-md-6">
                        <div class="card shadow">
                            <div class="card-header">
                                <h3 class="card-title">{{ $session }} - {{ request('date', now()->toDateString()) }}</h3>
                            </div>
                            <div class="card-body">
                                ...
                                @elseif(isset($sessionData['win_digit']))
                                    ...
                                    @if(isset($sessionData['winners']) && count($sessionData['winners']) > 0)
                                        <div class="table-responsive">
                                            <table class="table table-striped table-hover">
                                                ...
                                                <tbody>
                                                    @foreach($sessionData['winners'] as $winner)
                                                        @php
                                                            $breakGroup = array_sum(str_split($winner->bet_number));
                                                        @endphp
                                    ...
                                    @else
                                        <div class="alert alert-warning">
                                            <i class="fas fa-exclamation-triangle"></i> No winners found for this draw session.
                                        </div>
                                    @endif
                                @endif
                            </div>
        ...
        @elseif(isset($results))
            ...
            <div class="card shadow">
                <div class="card-header">
                    <h3 class="card-title">{{ request('draw_session', 'all') }} - {{ request('date', now()->toDateString()) }}</h3>
                </div>
                <div class="card-body">
                    @if(isset($results->win_number))
                        <div class="text-center mb-4">
                            <div class="alert alert-success">
                                ...
                                <p class="mb-0">
                                    <strong>Draw Session:</strong> {{ $results->draw_session }} | 
                    ...
                    @else
                        <div class="alert alert-info">
                            <i class="fas fa-info-circle"></i> No winning result found for this draw session and date.
                        </div>
                    @endif
                </div>
            </div>
            ...
            <div class="card shadow">
                <div class="card-body">
                    <div class="alert alert-info text-center">
                        <i class="fas fa-info-circle"></i> No winner data available for the selected date and draw session.
                    </div>
                </div>
            </div>
        @endif
                ...
                <div class="card-body">
                    <div class="row">
                        @php
                            ...
                            foreach($result as $session => $sessionData) {
                                if(isset($sessionData['winners'])) {
                                    $totalWinners += count($sessionData['winners']);
                                    foreach($sessionData['winners'] as $winner) {
